...
int main(int argc, char* argv[])
{
  ...
  config.add_options()
    ...
    ("no-data-change", "Do not change diffusion values")
  ...
  if(vm.count("help") || !vm.count("fiber-file"))
  {
    ...
    if(vm.count("help"))
    {
      std::cout << "Version $Revision$ "<< std::endl;
  ...
  for(it = children->begin(); it != children->end(); it++)
  {
    ...
    for(pit = pointlist.begin(); pit != pointlist.end(); ++pit)
    {
      ...
      if(vm.count("tensor-volume") && vm.count("fiber-output") && !vm.count("no-data-change"))
      {
