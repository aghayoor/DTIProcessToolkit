...
public:
  ...
  inline TOutput operator()( const TInput & x )
  {
    ...
    vnl_symmetric_eigensystem_compute_eigenvals(x[0], x[1], x[2],
                                                ...
                                                lambdas[1],
                                                lambdas[2]);
    // The resulting eigenvectors and values are sorted in increasing order, so
    // lambdas[0] is the smallest eigenvalue.
    // Now we order them based on magnitude
    double (*absolute)(double) = &std::abs;
    std::transform(lambdas, lambdas+3, lambdas, absolute);
    std::sort(lambdas, lambdas+3);
